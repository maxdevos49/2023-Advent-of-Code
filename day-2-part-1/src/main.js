...
function main(argv) {
	try {
		...
		if (Number.isNaN(maxRedCubes)) {
			throw new Error(
				'Unable to parse a valid integer for maximum red cubes.'
			);
		}
		const maxGreenCubes = Number.parseInt(argv[3]);
		if (Number.isNaN(maxGreenCubes)) {
			throw new Error(
				'Unable to parse a valid integer for maximum green cubes.'
			);
		}
		const maxBlueCubes = Number.parseInt(argv[4]);
		if (Number.isNaN(maxBlueCubes)) {
			throw new Error(
				'Unable to parse a valid integer for maximum blue cubes.'
			);
		}
		...
		for (const game of games) {
			if (
				game.redCubes <= maxRedCubes &&
				game.greenCubes <= maxGreenCubes &&
				game.blueCubes <= maxBlueCubes
			) {
				sum += game.id;
			}
		}
		...
		console.error(
			...
			} <max_red_cubes> <max_green_cubes> <max_blue_cubes> <path_to_game_data>\n\n`
		);
		process.exit(1);
	}
}
